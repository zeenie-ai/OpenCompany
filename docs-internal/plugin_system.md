...
**Retries on the deployed path.** The envelope also carries a `retryable` verdict (`services/plugin/retryability.py`), classified from the real exception: `NodeUserError`, Pydantic validation, credential, unknown-operation and Output-contract failures are permanent; `httpx` 4xx other than 408 / 425 / 429 are permanent; 5xx, timeouts, connection errors and unknown exceptions are transient; a boolean `retryable` attribute on the exception or its `__cause__` wins, which is how a `NodeUserError` raised by `ChatUnifier` from a rate-limited `LLMError` still retries. `BaseNode.as_activity` raises a typed `ApplicationError` for the envelope so Temporal applies `BaseNode.effective_retry_policy()`: a class-declared `retry_policy` wins; triggers and nodes annotated `destructive` or `readonly: False` (or not annotated at all) get one attempt; `readonly: True` keeps three. That makes `annotations["readonly"]` a contract: it means "safe to re-execute on a transient failure". `ctx.attempt` and `ctx.idempotency_key` (`f"{workflow_run_id}-{activity_id}"`, stable across attempts) are the hooks for making a retried write idempotent. `TEMPORAL_PLUGIN_FAILURE_RETRIES=false` restores the return-the-envelope behaviour; `tests/fixtures/effective_retry_attempts_snapshot.json` pins the attempt count per node type.
