...
async def dispatch_telegram_message_received(event_data: Mapping[str, Any]) -> int:
    ...
    Replaces the direct ``event_waiter.dispatch("telegram_message_received",
    event_data)`` call. Returns the count of resolved waiters.
    """
    from services import event_waiter

    # Trigger nodes subscribe by event_type string; keep the legacy
    # string so the `telegramReceive.event_type` ClassVar still
    # matches (Phase C1 + B11 will swap to the typed
    # `com.machinaos.telegram.message.received` form together).
    resolved = event_waiter.dispatch(
        _MESSAGE_LEGACY_EVENT_TYPE,
        dict(event_data),
    )
    return resolved
