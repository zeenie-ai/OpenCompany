...
Token VALUES are **hex + `color-mix()`** (not HSL triplets) and live in the per-theme files: [light.css](./client/src/themes/light.css) / [dark.css](./client/src/themes/dark.css) own the shadcn + dracula + node + action colour hex; [base.css](./client/src/themes/base.css) owns the shared scale / chrome / shadow / type tokens **plus the `--tint-*` alpha scale — the single home for `color-mix` percentages** (consumers reference `var(--tint-X)`; skins keep only their bespoke decorative alphas inline). [index.css](./client/src/index.css) is **plumbing-only**: its `@theme inline` bridge maps `--color-X: var(--X)` (NO `hsl()` wrapper). Tailwind v4 still composes alpha (`bg-primary/50`, `text-foreground/80`) by wrapping any colour in `color-mix()`, so utilities work against hex. Action intents also expose an **`--action-X-ink`** readable-text variant (light: a darkened accent; dark: the raw accent) — `ActionButton` reads `text-action-X-ink`. The **animation system** — per-theme `--pulse-keyframe` / `--pulse-duration` / `--pulse-timing` tokens, the executing + trigger keyframes, and the `.machina-pulse` / `.machina-trigger-armed` / `.machina-bolt` / `.machina-crt` / `.machina-scanline` helpers — lives in [animations.css](./client/src/themes/animations.css); base.css's `.executing` rule reads `var(--pulse-keyframe)` so a theme switches its pulse by overriding one token. Trigger nodes show a distinct continuous `.machina-trigger-armed` "listening" heartbeat (waiting state) vs the one-shot execution pulse, with the ⚡ badge pulsing via `.machina-bolt`.
...
3. **Action role tokens** — `--action-run`, `--action-stop`, `--action-save`, `--action-config`, `--action-secret`, `--action-tools`. `base / -soft / -hover / -border / -ink` set (e.g. `bg-action-run-soft text-action-run-ink border-action-run-border`, plus `hover:bg-action-run-hover`). `-ink` is the readable label colour (light: darkened accent; dark: raw accent). Used for toolbar icon buttons, File menu items, and as the tokens behind `<ActionButton>`'s `intent` variants. Themes redefine these without touching call sites; ActionButton composes hover via `-hover` (no opacity arithmetic).
...
Skill icons resolve through the same chain as the teaching node (mirrors `BaseNode._metadata_dict`): per-plugin `<plugin>/icon.svg` (or `<plugin>/icon_<type>.svg`) served as `/api/schemas/nodes/<type>/icon` → [`server/nodes/visuals.json`](./server/nodes/visuals.json) entry (emoji or `lobehub:<brand>`). Color falls through per-plugin `<plugin>/meta.json` then `visuals.json`. Resolution lives in [`server/services/skill_loader.py::_parse_skill_metadata`](./server/services/skill_loader.py) which calls `nodes._visuals.get_plugin_icon_path` / `get_plugin_meta` / `get_icon` / `get_color`. The pre-Wave-10 `skillNodes.ts` + `getNodeDefaults()` helper that did per-skill icon overrides on the frontend was retired; icons + colours now live exclusively on disk (per-plugin folder) with `visuals.json` as the fallback registry, and SKILL.md frontmatter is read only for orphan skills (no node target).
...
- **writeTodos**: Structured task list planning for complex multi-step operations. Connects to any agent's `input-tools` handle. Dual output (`tool` + `main`), dracula purple `#bd93f9`. Backed by `TodoService` singleton (`server/services/todo_service.py`) with JSON-based per-session state keyed by workflow_id. Schema: `WriteTodosSchema` with `TodoItem`/`TodoStatus` Pydantic enum (`pending` | `in_progress` | `completed`). Handler broadcasts `phase: "todo_update"` via WebSocket for real-time UI; `formatTodoOutput()` in `OutputDisplayPanel.tsx` renders as a checklist. Skill: `server/skills/assistant/write-todos-skill/SKILL.md` teaches the plan-work-update loop. **Parameter panel = editable Current Todos manager** — `uiHints.isTodoEditor` routes `MiddleSection` to [`client/src/components/parameterPanel/TodoEditor.tsx`](./client/src/components/parameterPanel/TodoEditor.tsx) instead of the empty generic-params card. It reads/writes the LIVE TodoService list (keyed `workflow_id` → `node_id`, so all writeTodos nodes in one workflow share one list) via the `get_todos` / `set_todos` WS handlers, with add / remove / edit-text / click-to-cycle-status. Both the tool's `write` op and manual panel edits emit the typed `todos_updated` CloudEvent through the centralized `services.events.dispatch.emit`; the FE handler refreshes the `['todos', session_key]` query live. Self-contained plugin folder `server/nodes/tool/write_todos/` (`__init__.py` + `_handlers.py` + `_events.py`).
...
- **montyExecutor**: Sandboxed Python via Pydantic Monty (`pydantic-monty`). Tool name: `sandboxed_python`. Deny-by-default with ENFORCED time/memory limits + dynamic `capabilities` (`http_get`/`workspace_read`/`workspace_write`). Supported subset only (no class/yield/with/match/random/collections).
...
### Code Nodes (4 nodes)
- **pythonExecutor**: **Dual-purpose node** - Execute Python code with syntax-highlighted editor, input_data access, and console output. Works as workflow node OR AI Agent tool (`python_code`). Available libraries: math, json, datetime, timedelta, re, random, Counter, defaultdict. CPython `exec()` with a restricted-builtins namespace; its `timeout` param is NOT enforced (only Temporal's CODE_EXEC activity timeout bounds it).
- **montyExecutor**: **Dual-purpose node** - Execute Python in a hard sandbox via Pydantic's [Monty](https://github.com/pydantic/monty) interpreter (`pydantic-monty==0.0.18`, pinned alpha). Works as workflow node OR AI Agent tool (`sandboxed_python`). Deny-by-default: no filesystem/network/env access unless the LLM requests it via the dynamic `capabilities` param (`http_get` → SSRF-guarded host fetch; `workspace_read`/`workspace_write` → `MountDir` over the per-workflow workspace at `/workspace`). **ENFORCED** `timeout` (→ `ResourceLimits.max_duration_secs`) and `max_memory_mb` (→ `max_memory` bytes) — the headline win over pythonExecutor. Supported subset: def/closures/lambda, comprehensions, f-strings, try/except, async def/await, and `import math|json|re`; NOT supported: class, yield/generators, with, match, `import random|collections|os`. Reads `input_data`; returns the last expression as `output` + captured `print()` as `console_output`. Lazy-imports `pydantic_monty` inside the op (graceful `NodeUserError` if the wheel is missing); plugin at [server/nodes/code/monty_executor/](./server/nodes/code/monty_executor/). Skill: `server/skills/coding_agent/monty-skill/SKILL.md`.
...
Cross-platform process manager for long-running subprocesses (dev servers, watchers, build tools). Uses `asyncio.create_subprocess_exec` with full system PATH (`env={**os.environ}`). Output streams to Terminal tab via `broadcast_terminal_log()` and persists to log files in the workspace. **Captured lines are ANSI-stripped at the source** — `process_service._read_stream` runs each decoded line through `core.ansi.strip_ansi` (a thin wrapper over **`click.unstyle`**, a declared dep) before the log-file write / Terminal broadcast / `get_output` / `line_handler`, so colour + cursor/erase codes from `vite`/`npm`/`git`/etc. don't render as garbage (`[36m…[39m`). The `shell` node strips its `stdout` the same way. `click.unstyle` was chosen over `rich.Text.from_ansi` because rich line-normalizes and drops trailing newlines; `click.unstyle` is byte-faithful except for the stripped escapes.
...
- **`_install.py`** — [`pooch`](https://pypi.org/project/pooch/) downloads the official `temporal` CLI archive from `https://temporal.download/cli/archive/latest?platform=<os>&arch=<arch>`. Cross-platform (Windows zip / macOS+Linux tar.gz), cached at `<DATA_DIR>/packages/temporal/`. Download uses an explicit `HTTPDownloader(timeout=300)` — the timeout is per-socket-read, not total transfer, so slow links can finish a ~114 MB fetch (pooch's 30 s default aborted them). Standalone entry: `python -m services.temporal._install` — invoked by `machina build` step [6/6] (fatal there, locked by `test_temporal_install_is_fatal_on_failure`) and by npm postinstall (**non-fatal** there: `TemporalServerRuntime._pre_spawn()` re-downloads lazily on first `machina start`, so a failed eager fetch must never fail `npm install -g`). Note dev mode resolves `DATA_DIR` to repo-local `.machina/`, so dev and `~/.machina` installs keep separate binary caches.
...
JWT handling uses **PyJWT** (`import jwt`, `jwt.encode` / `jwt.decode`, catch `jwt.PyJWTError`) — HS256 with `Settings.jwt_secret_key`. Do not reintroduce `python-jose`: it drags in pure-Python `ecdsa`, which carries an unpatchable Minerva timing-attack advisory (GHSA-wj6h-64fc-37mp).
...
3. **`--resume <UUID>` crash recovery.** If the warm subprocess dies between batches, [`ClaudeSessionPool.acquire`](./server/nodes/agent/claude_code_agent/_pool.py) detects `process.returncode is not None`, captures the dead session's `current_session_uuid` (last seen on a `result` event), and respawns with `--resume <captured_uuid>` so the same on-disk JSONL keeps growing. Mutually exclusive with `--continue` in `nodes/agent/claude_code_agent/_provider.py:interactive_argv`.
...
# result: {"total": 6000, "total_cost": 0.021, "threshold": 800000, "needs_compaction": False}  (80% of 1M)
...
# {"session_id": "user-123", "total": 15000, "threshold": 800000, "count": 1}  (80% of 1M)
